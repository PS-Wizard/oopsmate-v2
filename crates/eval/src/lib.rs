...
impl Evaluator for PestoEval {
    #[inline(always)]
    fn evaluate(&self, position: &Position) -> i32 {
        pesto_score(position)
    }
}

#[inline(always)]
fn pesto_score(position: &Position) -> i32 {
    ...
    for piece in PIECES {
        accumulate_piece_scores(board, piece, &mut mg, &mut eg, &mut game_phase);
    }
...
fn accumulate_piece_scores(
    ...
    let piece_bb = board.piece_bb(piece);
    let white_pieces = piece_bb & board.color_bb(Color::White);
    let black_pieces = piece_bb & board.color_bb(Color::Black);
    let mg_table = MG_TABLES[piece_index];
    let eg_table = EG_TABLES[piece_index];
    let mg_value = MG_VALUE[piece_index];
    let eg_value = EG_VALUE[piece_index];
    let phase_increment = GAME_PHASE_INC[piece_index];

    accumulate_side_scores(
        white_pieces,
        0,
        false,
        mg,
...
fn accumulate_side_scores(
    ...
    while pieces != 0 {
        ...
        let table_square = if mirror { square ^ 56 } else { square };
        mg[color_index] += mg_value + mg_table[table_square];
        eg[color_index] += eg_value + eg_table[table_square];
        *game_phase += phase_increment;
    }
}
...
mod tests {
    ...
    fn start_position_is_equal() {
        ...
        assert_eq!(PestoEval.evaluate(&pos), 0);
    }
    ...
    fn score_is_from_side_to_move_perspective() {
        ...
        assert_eq!(PestoEval.evaluate(&white), -PestoEval.evaluate(&black));
    }

    #[test]
    fn central_knight_scores_better_than_corner_knight() {
        ...
        assert!(PestoEval.evaluate(&center) > PestoEval.evaluate(&corner));
    }
}
